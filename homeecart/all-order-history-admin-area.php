...
<h3><center>All Orders History</center></h3>
...
$query = $con->query("SELECT order_payments.amount, order_payments.transaction_id, order_details.item_id, order_details.order_id, order_details.username, order_details.basic_price, order_details.quantity, order_details.gst_rate, order_details.packing_cost, order_details.shipping_cost, order_details.bill, order_details.order_date, order_details.status FROM order_payments INNER JOIN order_details ON order_payments.order_id=order_details.order_id");

/*inventory_details.item_name, */

/*member.name as dropshipper, member.plan_code, member.email as dropshipper_email, member.mobile as dropshipper_mobile,*/

/*, customer_details.name as customer_name, customer_details.mobile, customer_details.street, customer_details.city, customer_details.state, customer_details.pincode, customer_details.email*/
...
if($order_status=='0') $order_status='<b style="color:red;">Pending</b>'; 
elseif($order_status=='1') $order_status='<b style="color:orange;">Received</b>'; 
elseif($order_status=='2') $order_status='<b style="color:green;">Dispatched</b>'; 
...
echo'</br><h4>No order found. Please wait until you get a new order.</h4>';

if(isset($_GET['confirm_dispatch_order'])){
