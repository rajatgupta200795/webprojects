<?php include"./inc/header.php";  ?>


<?php

$query = $con->query("SELECT * FROM student_entry WHERE roll_no='111731'");

if($query->num_rows){
while($row = $query->fetch_assoc()){

$fname = ucwords($row['first_name']);
$lname = ucwords($row['last_name']);
$roll_no = $row['roll_no'];
$class = $row['present_class'];
$father_name = ucwords($row['father_name']);
$mother_name = ucwords($row['mother_name']);
$dob = $row['birth_date'].'/'.$row['birth_month'].'/'.$row['birth_year'];

    
$student_photo = $row['student_photo'];
$gender = $row['gender'];
if($student_photo==""){
if($gender=="male") $student_photo="http://mcskkt.com/img/default_male.png";
else $student_photo="http://mcskkt.com/img/default_female.png";
}
}
}

echo'
<center><h1>Result Details</h1></center>
<hr style="height:1px;border:none;color:#333;background-color:#333;">
</br>


<div id="marksheet" style="border:10px solid skyblue; padding:10px; media:print; ">
<div style="border:2px solid black;">
<div style="padding:20px;">  
<img src="http://www.mcskkt.com/img/img2.jpg" style="position:absolute; left:76%; width:20%; height:auto; border:1px solid black;">
</div>  
<div style="font-weight:bold; text-align:center; line-height:35px;">
<div style="font-family: calibri; font-size:30px; margin-top:-20px;">MAHAVEER CONVENT SCHOOL</div>
<div style="font-family:Helvetica;">Shri Ram Chauk ,Kubersthan Road Kathkuiyan Bazar</br>
Affiliated to CBSC, New Delhi</div>

</div>

</br>
</br>
</br>

<div style="background-color:grey; width:100%; border-bottom:2px solid black; text-align:center; padding:7px; font-size:25px; font-weight:bold;">SESSION 2017-2018</div>

<div style="padding:10px; line-height:35px;">

<div class="row">
<div class="col-sm-4" style="text-align:center;">
<img src="'.$student_photo.'" style="height:150px; width:auto; border:1px solid black;">
</div>
<div class="col-sm-8">

<div class="row">
<div class="col-sm-5">
<b>Student Name :</b> '.$fname.' '.$lname.'
</div>
<div class="col-sm-3">
<b>Class : </b> '.$class.'
</div>
<div class="col-sm-3">
<b>Student Id :</b> '.$roll_no.'
</div>
</div>

<div class="row">
<div class="col-sm-5">
<b>Father\'s Name :</b> '.$father_name.'
</div>
<div class="col-sm-3">
<b>DOB. : </b> '.$dob.'
</div>
<div class="col-sm-3">
<b></b>
</div>
</div>
</div>

</div>
</div>

<div style="height:2px; background-color: black; width:100%;"></div>

<div style="text-align:center; padding:10px; font-weight:bold; font-family:courier; font-size:20px; border-bottom:2px solid black;">ACADEMIC PERFORMANCE : SCHOLASTIC AREA</div>

<div style="background-color: #f0f0f0; width:100%; border-bottom:2px solid black; text-align:center; padding:7px; font-size:25px; font-weight:bold; font-family: Georgia;">Formative Assessment-3 (FA-3)</div>


<style>
.row.div{
    border:2px solid black;
}
</style>

<div class="row" style="padding:10px;">
<div class="col-sm-1">
<b>Sr. No.</b>
</div>
<div class="col-sm-5">
<b>Subject name</b>
</div>
<div class="col-sm-1">
<b>Obtained Marks</b>
</div>
<div class="col-sm-1">
<b>Original Marks</b>
</div>
<div class="col-sm-1">
<b>Weightage (10%)</b>
</div>
<div class="col-sm-1">
<b>Percentage</b>
</div>
<div class="col-sm-1">
<b>Grade</b>
</div>
<div class="col-sm-1">
<b>Grade Point</b>
</div>
</div>
<div style="height:2px; background-color: black; width:100%;"></div>

<div class="row" style="padding:10px;">
<div class="col-sm-1">
1.
</div>
<div class="col-sm-5">
Subject name
</div>
<div class="col-sm-1">
Obtained Marks
</div>
<div class="col-sm-1">
Original Marks
</div>
<div class="col-sm-1">
Weightage (10%)
</div>
<div class="col-sm-1">
Percentage
</div>
<div class="col-sm-1">
Grade
</div>
<div class="col-sm-1">
Grade Point
</div>
</div>
<div style="height:2px; background-color: black; width:100%;"></div>



</div>

</div>
';




?>






<?php include"./inc/footer.php";  ?>
