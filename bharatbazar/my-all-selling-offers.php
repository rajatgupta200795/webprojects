...
$blocked_count=0;
$pending_count=0;
$query=$con->query("SELECT * from sell_offer where username='$username' order by status asc"); 
...
elseif($status==-1)$blocked_count++;
elseif($status==0)$pending_count++;
}
?>
...
<center>
<div style="padding-bottom:110px; width: 84%; background-color: white; border-radius:5px; font-weight:normal; color: black; text-align: left; font-size: 17px; font-weight: bold; font-family: \'PT Sans Narrow\', sans-serif; line-height: 30px;">
...
if($status==1) echo'<div class="col-sm-1"><a href="?product_id='.$product_id.'&stop_sell_offer=true" style="color:red;">Stop</a></div>';
elseif($status==2) echo'<div class="col-sm-1"><a href="?product_id='.$product_id.'&renew_sell_offer=true" style="color:green;">Renew</a></div>';
...
if(isset($_GET['renew_sell_offer'])){
$product_id = $_GET['product_id'];
$query =  $con->query("UPDATE sell_offer set status='1' where product_id='$product_id'");
if($query)
header("location:my-all-selling-offers.php");
else echo'<script>alert("Not Renew : Error Found");</script>';
}

if(isset($_GET['stop_sell_offer'])){
$product_id = $_GET['product_id'];
$query =  $con->query("UPDATE sell_offer set status='2' where product_id='$product_id'");
if($query)
header("location:my-all-selling-offers.php");
